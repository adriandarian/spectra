...
API reference for md2jira's plugin hooks system.

## Overview

The hooks system allows you to execute custom code at specific points during md2jira operations. This enables:
...
```python
from md2jira.plugins import HookPoint
...
def alert_pagerduty(ctx):
    ...
    if isinstance(ctx.error, (AuthenticationError, ConnectionError)):
        pagerduty.trigger(
            summary=f"md2jira sync failed: {ctx.error}",
            severity="error",
            source="md2jira",
            custom_details={

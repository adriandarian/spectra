...
md2jira uses standard Unix exit codes to indicate how the program terminated. These codes can be used in shell scripts for automation and CI/CD pipelines.
...
```bash
md2jira --markdown nonexistent.md --epic PROJ-123
echo $?  # Returns 3
```
...
```bash
md2jira --validate --markdown invalid.md --epic PROJ-123
echo $?  # Returns 6
```
...
```bash
md2jira --markdown EPIC.md --epic RESTRICTED-123 --execute
echo $?  # Returns 7
```
...
```bash
md2jira --markdown EPIC.md --epic INVALID-999 --execute
echo $?  # Returns 8
```
...
The operation completed, but some individual operations failed. With graceful degradation enabled, md2jira continues processing even when some issues fail.
...
- name: Sync to Jira
  id: sync
  run: |
    md2jira --markdown EPIC.md --epic ${{ env.EPIC_KEY }} --execute --no-confirm
...
md2jira follows standard Unix conventions:

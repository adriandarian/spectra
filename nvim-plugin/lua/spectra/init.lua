-- spectra.nvim - Neovim plugin for spectra
...
-- Get the spectra executable path
function M._get_executable()
  return M.config.executable or "spectra"
end
...
function M.dashboard(opts)
  ...
  M._show_output(result.stdout, "spectra Dashboard")
end
...
function M.init()
  local args = { "--init" }
  local result = M.run_sync(args)
  M._show_output(result.stdout, "spectra Setup")
end
...
function M._notify(msg, level)
  if M.config.notify then
    vim.notify(msg, level, { title = "spectra" })
  end
end
...
function M._setup_keymaps()
  ...
  if km.validate then
    vim.keymap.set("n", km.validate, M.validate, { desc = "spectra: Validate" })
  end

  if km.sync then
    vim.keymap.set("n", km.sync, M.sync, { desc = "spectra: Sync (dry-run)" })
  end

  if km.sync_execute then
    vim.keymap.set("n", km.sync_execute, M.sync_execute, { desc = "spectra: Sync (execute)" })
  end

  if km.dashboard then
    vim.keymap.set("n", km.dashboard, M.dashboard, { desc = "spectra: Dashboard" })
  end

  if km.stories then
    vim.keymap.set("n", km.stories, M.telescope_stories, { desc = "spectra: Find stories" })
  end

  if km.init then
    vim.keymap.set("n", km.init, M.init, { desc = "spectra: Init" })
  end
end

-- Setup autocommands
function M._setup_autocmds()
  local group = vim.api.nvim_create_augroup("spectra", { clear = true })
...
function M.telescope_stories()
  ...
  require("spectra.telescope").stories()
end

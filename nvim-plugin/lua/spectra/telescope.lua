-- Telescope integration for spectra
...
function M.stories(opts)
  ...
  if #stories == 0 then
    vim.notify("No stories found in current buffer", vim.log.levels.WARN, { title = "spectra" })
    return
  end

  pickers.new(opts, {
    prompt_title = "spectra Stories",
    ...
    attach_mappings = function(prompt_bufnr, map)
      ...
      map("i", "<C-s>", function()
        ...
        local selection = action_state.get_selected_entry()
        if selection then
          spectra.sync({ story = selection.value.id })
        end
      end)
...
function M.epics(opts)
  ...
  if #epics == 0 then
    vim.notify("No epics found in current buffer", vim.log.levels.WARN, { title = "spectra" })
    return
  end

  pickers.new(opts, {
    prompt_title = "spectra Epics",
    finder = finders.new_table({
      results = epics,
      entry_maker = function(entry)
        return {
...
-- Commands picker (all spectra commands)
function M.commands(opts)
  opts = opts or {}

  local commands = {
    { name = "Validate", desc = "Validate markdown file", fn = spectra.validate },
    { name = "Sync (dry-run)", desc = "Preview sync changes", fn = spectra.sync },
    { name = "Sync (execute)", desc = "Execute sync changes", fn = spectra.sync_execute },
    { name = "Dashboard", desc = "Show status dashboard", fn = spectra.dashboard },
    { name = "Init", desc = "Run setup wizard", fn = spectra.init },
    { name = "Find Stories", desc = "Browse stories", fn = M.stories },
  }

  pickers.new(opts, {
    prompt_title = "spectra Commands",
    finder = finders.new_table({
      results = commands,
      entry_maker = function(entry)
        return {

...
class Schedule(ABC):
    ...
    def seconds_until_next(self, after: datetime = None) -> float:
...
class CronSchedule(Schedule):
    ...
    def next_run_time(self, after: datetime = None) -> datetime:
        ...
        for _ in range(7 * 24 * 60):  # Max iterations
            ...
            if (current.weekday() in self.days_of_week and
                current.hour in self.hours and
...
class ScheduledSyncRunner:
    ...
    def __init__(
        ...
        on_run_complete: Optional[Callable[["SyncResult"], None]] = None,
        ...
        self._current_run: Optional[threading.Thread] = None
    ...
    def start(self) -> None:
        ...
        try:
            while self._running and not self._stop_event.is_set():
                ...
                self.logger.info(f"Next sync at {next_run.strftime('%Y-%m-%d %H:%M:%S')} "
...
class ScheduleDisplay:
    ...
    def show_run_complete(self, result: "SyncResult") -> None:
        ...
        if result.success:
            ...
            if not self.quiet:
                print(f"   Stories: {result.stories_matched} matched, "

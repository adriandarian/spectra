    ...
    def _watch_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                self._notify_change(FileChange(
                    path=str(self.path),
...
class WatchOrchestrator:
    ...
    def __init__(
        ...
        on_change_detected: Optional[Callable[[FileChange], None]] = None,
    ):
        """
    ...
    def stop(self) -> None:
        ...
        self.logger.info(f"Session stats: {self.stats.syncs_triggered} syncs, "
    ...
    def _trigger_sync(self) -> None:
        ...
        try:
            ...
            if result.success:
                ...
                self.logger.info(f"Sync completed: {result.stories_updated} stories updated, "
...
class WatchDisplay:
    ...
    def show_sync_complete(self, result: "SyncResult") -> None:
        ...
        if result.success:
            ...
            if not self.quiet:
                print(f"   Stories: {result.stories_matched} matched, {result.stories_updated} updated")
                print(f"   Subtasks: {result.subtasks_created} created, {result.subtasks_updated} updated")
        else:

...
class PullChanges:
    ...
    def total_changes(self) -> int:
        ...
        return (
            len(self.new_stories) +
            len(self.updated_stories) +
            len(self.deleted_stories)
        )
...
class ReverseSyncOrchestrator:
    ...
    def _extract_story_id(self, summary: str) -> Optional[str]:
    ...
    def _parse_jira_description(self, description: Any) -> Optional[Description]:
        ...
        import re
        pattern = r'[Aa]s\s+a\s+(.+?)[,.]?\s+[Ii]\s+want\s+(.+?)[,.]?\s+[Ss]o\s+that\s+(.+?)(?:\.|$)'
    ...
    def _detect_changes(
        ...
        existing_story_ids = {str(s.id) for s in self._md_stories}
        ...
        for jira_story in jira_stories:
            ...
            if not matched_id:
                ...
                # Check for updates
                md_story = next(
                    (s for s in self._md_stories if str(s.id) == matched_id),
                    None
                )

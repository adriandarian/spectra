...
class AsyncJiraAdapter(AsyncIssueTrackerPort):
    ...
    def _ensure_connected(self) -> AsyncJiraApiClient:
        ...
        if self._client is None:
            raise RuntimeError(
                "Async adapter not connected. Use 'async with' context manager."
            )
        return self._client
    ...
    async def get_issues_async(self, issue_keys: Sequence[str]) -> list[IssueData]:
        ...
        results = await client.get_issues_parallel(
            list(issue_keys),
            fields=[fields]
        )
    ...
    ) -> list[tuple[Optional[str], bool, Optional[str]]]:
...
__all__ = ["AsyncJiraAdapter", "is_async_available", "ASYNC_AVAILABLE"]

...
"""
    ...
    def _show_completion(self) -> None:
        ...
        if self.config.create_sample:
            ...
            self.console.detail(f"  spectra --markdown {self.config.sample_path} --epic {self.config.project_key or 'PROJ'}-123")
            self.console.item("Execute the sync when ready:")
            self.console.detail(f"  spectra --markdown {self.config.sample_path} --epic {self.config.project_key or 'PROJ'}-123 --execute")
        else:
            ...
            self.console.item("Execute when ready: spectra --markdown EPIC.md --epic PROJ-123 --execute")

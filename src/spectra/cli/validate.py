...
class MarkdownValidator:
    ...
    def _check_structure(
        ...
        story_ids = [m.group(1) for m in story_matches]
    ...
    def _check_best_practices(
        ...
        has_epic_header = bool(re.search(r'^#\s+.*(?:Epic|🚀)', content, re.MULTILINE | re.IGNORECASE))
        ...
        trailing_ws_count = sum(1 for line in lines if line.endswith(" ") or line.endswith("\t"))
...
def format_validation_result(result: ValidationResult, color: bool = True) -> str:
    ...
    if result.warnings:
        if color:
            lines.append(f"{Colors.YELLOW}{Colors.BOLD}Warnings ({len(result.warnings)}):{Colors.RESET}")
        else:

...
Supports:
- .md2jira.yaml / .md2jira.yml
- .md2jira.toml
- pyproject.toml [tool.md2jira] section
...
class FileConfigProvider(ConfigProviderPort):
    ...
    def _find_config_file(self) -> Path | None:
        ...
        for base_path in search_paths:
            ...
            if pyproject.exists() and self._has_md2jira_section(pyproject):
                return pyproject

        return None

    def _has_md2jira_section(self, pyproject_path: Path) -> bool:
        """Check if pyproject.toml has [tool.md2jira] section."""
        if tomllib is None:
            return False
        ...
        try:
            with pyproject_path.open("rb") as f:
                data = tomllib.load(f)
            return "md2jira" in data.get("tool", {})
        except Exception:
            return False
    ...
    def _load_pyproject_toml(self, path: Path) -> None:
        """Load config from pyproject.toml [tool.md2jira] section."""
        if tomllib is None:
            raise ConfigFileError(
                path,
                "TOML support not available. Install with: pip install tomli",
        ...
        try:
            with path.open("rb") as f:
                data = tomllib.load(f)
            md2jira_config = data.get("tool", {}).get("md2jira", {})
            if md2jira_config:
                self._values = md2jira_config
        except Exception as e:
            raise ConfigFileError(path, f"Invalid TOML syntax: {e}") from e

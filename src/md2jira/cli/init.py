"""
Init Wizard - First-time setup assistant for md2jira.
...
class InitWizard:
    """
    Interactive setup wizard for first-time md2jira configuration.
...
'''
    ...
    def run(self) -> bool:
        ...
        if not self._test_connection():
            if not self._prompt_continue_anyway():
                return False
    ...
    def _show_welcome(self) -> None:
        """Show welcome banner."""
        self.console.header(f"md2jira Setup Wizard {Symbols.GEAR}")
        self.console.print()
        self.console.info("Welcome! This wizard will help you set up md2jira.")
    ...
    def _check_existing_config(self) -> bool:
        ...
        if os.environ.get("JIRA_URL") or os.environ.get("JIRA_API_TOKEN"):
            return True
    ...
    def _test_connection(self) -> bool:
        ...
        try:
            from ..adapters import JiraAdapter, ADFFormatter
    ...
    def _choose_config_format(self) -> None:
        ...
        options = [
            ...
            (ConfigFormat.YAML, ".md2jira.yaml", "Human-readable YAML format"),
            (ConfigFormat.TOML, ".md2jira.toml", "Modern TOML format"),
        ]
        
        for i, (fmt, name, desc) in enumerate(options, 1):
    ...
    def _create_env_file(self) -> None:
        ...
        content = f"""# md2jira Configuration
# Generated by md2jira init
...
"""
    ...
    def _create_yaml_file(self) -> None:
        """Create .md2jira.yaml configuration file."""
        path = Path(".md2jira.yaml")
...
"""
        ...
        # Add to .gitignore if it exists
        self._add_to_gitignore(".md2jira.yaml")
    
    def _create_toml_file(self) -> None:
        """Create .md2jira.toml configuration file."""
        path = Path(".md2jira.toml")
...
"""
        ...
        # Add to .gitignore if it exists
        self._add_to_gitignore(".md2jira.toml")
    ...
    def _add_to_gitignore(self, filename: str) -> None:
        ...
        with gitignore.open("a") as f:
            if not content.endswith("\n"):
                f.write("\n")
            f.write(f"\n# md2jira configuration (contains credentials)\n")
            f.write(f"{filename}\n")
    ...
    def _show_completion(self) -> None:
        ...
        self.console.success("md2jira is ready to use!")
        self.console.print()
        ...
        if self.config.create_sample:
            ...
            self.console.item(f"Create the epic in Jira and note the key (e.g., {self.config.project_key or 'PROJ'}-123)")
            self.console.item("Run a dry-run to preview changes:")
            self.console.detail(f"  md2jira --markdown {self.config.sample_path} --epic {self.config.project_key or 'PROJ'}-123")
            self.console.item("Execute the sync when ready:")
            self.console.detail(f"  md2jira --markdown {self.config.sample_path} --epic {self.config.project_key or 'PROJ'}-123 --execute")
        else:
            self.console.item("Create a markdown file with your epic content")
            self.console.item("Run a dry-run: md2jira --markdown EPIC.md --epic PROJ-123")
            self.console.item("Execute when ready: md2jira --markdown EPIC.md --epic PROJ-123 --execute")
        
        self.console.print()
        self.console.info("For more information:")
        self.console.detail("  Documentation: https://md2jira.dev")
        self.console.detail("  Examples: md2jira --help")

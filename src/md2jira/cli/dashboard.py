...
class Dashboard:
    """
    TUI Dashboard for md2jira sync status.
    ...
    def _build_header(self, data: DashboardData) -> list[str]:
        ...
        # Title bar
        title = f" md2jira Dashboard {Symbols.ROCKET} "
        if self.color:
            width = 70
    ...
    def _build_stats_row(self, data: DashboardData) -> list[str]:
        ...
        if self.color:
            ...
            lines.append(f"  Stories: {total_stories}  |  Points: {total_points}  |  Subtasks: {total_subtasks}  |  Syncs: {data.total_syncs}")
    ...
    def _build_sync_status(self, data: DashboardData) -> list[str]:
        ...
        if self.color:
            lines.append(f"  {Colors.BOLD}Last Sync:{Colors.RESET}", )
        else:
        ...
        if data.total_syncs > 0:
            ...
            if self.color:
                ...
                lines.append(f"    Success rate: {rate_color}{rate:.0f}%{Colors.RESET} ({data.successful_syncs}/{data.total_syncs})")
            else:
                lines.append(f"    Success rate: {rate:.0f}% ({data.successful_syncs}/{data.total_syncs})")
    ...
    def _build_stories_summary(self, data: DashboardData) -> list[str]:
        ...
        if self.color:
            ...
            lines.append(f"    ✓ {synced} synced | ◯ {pending} pending | ✗ {errors} errors | ↻ {changed} changed")
        
        # Show first few stories
        lines.append("")
        max_show = 8
        for i, story in enumerate(data.stories[:max_show]):
            ...
            if self.color:
                ...
                lines.append(f"    {status_icon} {story.story_id}: {title_display}{Colors.DIM}{jira_info}{Colors.RESET}")
            else:
                title_display = story.title[:40] + "..." if len(story.title) > 40 else story.title
    ...
    def _render_sessions(self, data: DashboardData) -> None:
        ...
        if data.has_pending_session:
            progress = data.completed_operations / max(data.pending_operations + data.completed_operations, 1)
            ...
            if self.color:
                ...
                print(f"  {bar} {pct}%")
                print(f"  {Colors.DIM}Completed: {data.completed_operations} | Pending: {data.pending_operations}{Colors.RESET}")
            else:
                print(f"  ⚡ Active Session")
                print(f"  Progress: {pct}%")
                print(f"  Completed: {data.completed_operations} | Pending: {data.pending_operations}")
        else:
            if self.color:
    ...
    def _render_footer(self) -> None:
        ...
        if self.color:
            ...
            print(f"{Colors.DIM}  Last updated: {datetime.now().strftime('%H:%M:%S')}  |  md2jira dashboard{Colors.RESET}")
        else:
            print("-" * 70)
            print(f"  Last updated: {datetime.now().strftime('%H:%M:%S')}  |  md2jira dashboard")
...
def load_dashboard_data(
    ...
    if markdown_path and Path(markdown_path).exists():
        try:
            from ..adapters import MarkdownParser

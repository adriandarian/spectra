...
class StoryFingerprint:
    ...
    def has_any_changes(self, other: "StoryFingerprint") -> bool:
        ...
        if self.metadata_hash != other.metadata_hash:
            return True
        if self.subtask_hashes != other.subtask_hashes:
            return True
        return False
...
class ChangeDetectionResult:
    ...
    def subtasks_changed(self) -> bool:
        ...
        return bool(
            self.subtasks_added or 
            self.subtasks_removed or 
            self.subtasks_modified
        )
...
class ChangeTracker:
    ...
    Example:
        >>> tracker = ChangeTracker(storage_dir="~/.md2jira/sync")
        >>> 
        >>> # Load previous state
    ...
    def __init__(
        self,
        storage_dir: str | Path = "~/.md2jira/sync",
    ):
        """
    ...
    def load(self, epic_key: str, markdown_path: str) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Loaded {len(self._previous_fingerprints)} fingerprints "
                f"from previous sync"
            )
            return True
    ...
    def detect_changes(
        ...
        self.logger.info(
            f"Change detection: {changed_count}/{len(stories)} stories changed"
        )
    ...
    def save(self, epic_key: str, markdown_path: str) -> None:
        ...
        data = {
            ...
            "fingerprints": [
                fp.to_dict() for fp in self._current_fingerprints.values()
            ],
        }
        
        state_path.write_text(json.dumps(data, indent=2))
        self.logger.info(
            f"Saved {len(self._current_fingerprints)} fingerprints"
        )
    ...
    def get_changed_story_ids(
        ...
        return {
            story_id for story_id, result in changes.items()
            if result.has_changes
        }
